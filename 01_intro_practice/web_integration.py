...
mlModelPath = os.path.join(absDir, 'military_vehicle_classifier_mk1.pkl')
mlModel = load_learner(mlModelPath)
...
class FileDemo(object):
    ...
    def upload(self, myFile):
        ...
        allData = myFile.file.read()
        size = len(allData)
        prediction = mlModel.predict(allData)
        
        return out % (size, myFile.filename, myFile.content_type, prediction)
